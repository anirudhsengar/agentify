...
esbuild (root)                       0.28.1
tsx                                  4.23.1
esbuild (tsx nested)                 none; deduplicated to root
...
| Required field | Compatibility assessment |
| --- | --- |
| Baseline version | esbuild range `^0.25.12`, resolved 0.25.12; tsx range `^4.20.0`, resolved 4.22.4. The baseline tsx subtree separately carried esbuild 0.28.1. |
| Implemented version | esbuild range `^0.28.1`, resolved 0.28.1, and tsx range `^4.23.1`, resolved 4.23.1, as one coherent build-tooling group. |
...
| Breaking API/type changes | esbuild 0.28.0 is intentionally a breaking release. Existing Agentify build options required no adaptation. tsx remains on 4.x and officially depends on esbuild `~0.28.0`. |
| Runtime behavior changes | esbuild owns the compiled CLI; tsx executes repository tests and scripts. No runtime, format, target, minification, code-splitting, public-export, asset-copy, or executable-entry behavior changed. |
| Node requirements | Both implemented packages require Node >=18, below Agentify's Node 22.19.0 floor. |
...
| Bundling implications | The deterministic CLI bundle increased by 5,388 bytes (0.036%) and its sourcemap by 273 bytes. Node 22.19.0 and Node 24.13.1 emitted identical candidate hashes; copied prompts/workflows and the 181-file package inventory were unchanged. |
| Schema implications | Bundled schema identity and the focused schema-composition/ownership tests remained green. |
| Tool/session API implications | Pi and TypeBox runtime boundaries remained on their existing package versions and were not duplicated or rewritten by the deduplicated build graph. |
| Security advisory effects | esbuild 0.28 adds npm/Deno binary integrity checks. 0.28.1 includes a Windows development-server traversal fix; Agentify does not use the development server. Production audit remained at zero vulnerabilities. |
| Lockfile impact | Root esbuild plus its 26 platform packages moved from 0.25.12 to 0.28.1; tsx moved from 4.22.4 to 4.23.1; tsx's nested esbuild plus 26 nested platform packages were removed through deduplication. No production dependency moved. |
...
| Decision | **Implemented in #61 as one dependency group.** |

Implementation result (2026-07-15):

- clean `npm ci`, typecheck, build, focused schema ownership/composition tests, documentation invariants, package audit, pack, and installed-package checks passed on exact Node 22.19.0 and Node 24.13.1;
- `dist/cli.js` and its sourcemap were deterministic across both runtimes;
- the installed CLI preserved package identity, version/help behavior, deep-import rejection, executable entry, runtime assets, and the 181-file package boundary;
- the packed artifact was 8,050,846 bytes, unpacked to 45,718,774 bytes, and had SHA-1 `8fb4d623e762b3a81906b226172237c8acbaf18d`; and
- no temporary discovery workflow or script was retained.
...
| Required field | Compatibility assessment |
| --- | --- |
| Current version | Product engine `>=22.19.0`; required CI on Node 22.19.0 and 24. |
...
| esbuild 0.28.1 + tsx 4.23.1 | 0 | 27 | 28 | Implemented in #61; deduplicates tsx's nested esbuild/platform tree into the root candidate. |
