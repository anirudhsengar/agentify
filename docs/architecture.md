...
The orchestrator plans but cannot edit application source. Specialists provide
read-only evidence. One builder receives path-bounded write tools on an isolated
branch. Validation runs in trusted process code, not through the model. The
reviewer is role-separated from the builder and read-only. The knowledge maintainer may
write only validated Agentify knowledge paths.
