...
function verdictParameters(parameters) {
 const schema=structuredClone(parameters);
 schema.required=schema.required.filter(name=>name!=='checked_claims');
 schema.properties.checked_claims.description='Approval checklist only. Required and complete for supported. Omit for unsupported; an exact-source rejection does not require approving other claims.';
 return schema;
}
const sdk=new PiSdkRuntime();
const runtime={async runSession(options){const observe=options.onEvent;return sdk.runSession({...options,systemPrompt:options.systemPrompt+' The checked_claims checklist is approval evidence only. For an unsupported verdict, stop at the first decisive source finding and omit checked_claims. Do not finish checking unrelated claims before reporting a rejection. Supported still requires every supplied claim ID.',customTools:options.customTools?.map(tool=>({...tool,parameters:verdictParameters(tool.parameters),async execute(...args){const raw=args[1];const normalized=raw.verdict==='unsupported'&&raw.checked_claims===undefined?{...raw,checked_claims:[]}:raw;const result=await tool.execute(args[0],normalized,...args.slice(2));if(tool.name==='submit_specialist_review')acceptedSubmissions.push(structuredClone(args[1]));return result;}})),onEvent(event){if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
