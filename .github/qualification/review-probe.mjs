...
const submittedBodyDigest=specialistReviewDigest(body);const acceptedSubmissions=[];const sourceObservations=[];
...
const runtime={async runSession(options){const observe=options.onEvent;const task=JSON.parse(options.userPrompt);reviewTasks.push({thinking_level:options.config.thinkingLevel,source_observation:task.source_observation===true,source_precheck:task.source_precheck===true,source_excerpt:task.source_excerpt===true,claim_ids:Object.keys(task.claims??{}),source_paths:Object.keys(task.evidence),source_bytes:Object.values(task.evidence).reduce((n,s)=>n+Buffer.byteLength(s),0),output_cap:options.maxOutputTokens,timeout_ms:options.timeoutMs});return sdk.runSession({...options,customTools:options.customTools?.map(tool=>({...tool,async execute(...args){const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push(structuredClone(args[1]));if(tool.name==='submit_source_observations')sourceObservations.push(structuredClone(args[1]));return result;}})),onEvent(event){if(event.type==='tool_execution_start')sdkCalls.push({id:event.toolCallId,args:event.args});if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
...
assert.ok(result.source_unchanged);if(selected.require_source_excerpt)assert.ok(reviewTasks.some(t=>t.source_observation&&t.source_excerpt),'Required bounded source excerpt did not execute');if(selected.require_source_observation)assert.ok(reviewTasks.some(t=>t.source_observation&&t.claim_ids.length===0),'Required source-only observation stage did not execute');assert.ok(provider_wire.length>0&&provider_wire.every((r,index)=>r.model==='MiniMax-M3'&&r.tool_choice?.type==='auto'&&[4096,12000].includes(r.max_tokens)&&(reviewTasks[index]?.source_observation?r.thinking?.type==='disabled'||!r.thinking:r.thinking?.type==='adaptive')),'Actual wire did not preserve documented thinking mode and output cap');assert.ok(result.positive_control_passed,'Valid cache control was rejected or changed');assert.ok(!result.known_bad_body_approved,'Known source contradiction was approved');if(selected.false_claim_canary)assert.ok(result.canary_rejected,'false numeric-expiry canary was not explicitly rejected');else assert.ok(result.valid_typed_outcome,'bounded review did not reach an explicit typed outcome');
