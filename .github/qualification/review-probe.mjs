import {verifyBalancedAssignments} from './verify-balanced-assignments.mjs';
...
const runtime={async runSession(options){const observe=options.onEvent;const task=readReviewPrompt(options.userPrompt);reviewTasks.push({assignment:task.assignment??null,source_digests:Object.fromEntries(Object.entries(task.evidence).map(([file,bytes])=>[file,createHash('sha256').update(bytes).digest('hex')])),thinking_level:options.config.thinkingLevel,source_precheck:task.source_precheck===true,source_excerpt:task.source_excerpt===true,claim_ids:Object.keys(task.claims),source_paths:Object.keys(task.evidence),source_bytes:Object.values(task.evidence).reduce((n,s)=>n+Buffer.byteLength(s),0),output_cap:options.maxOutputTokens,timeout_ms:options.timeoutMs});return sdk.runSession({...options,customTools:options.customTools?.map(tool=>({...tool,async execute(...args){const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push({...structuredClone(args[1]),assignment_index:task.assignment?.index,body_digest:task.assignment?.body_digest});return result;}})),onEvent(event){if(event.type==='tool_execution_start')sdkCalls.push({id:event.toolCallId,args:event.args});if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
...
if(selected.require_complete_assignments)verifyBalancedAssignments(result);
