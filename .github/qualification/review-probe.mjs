...
function readableEvidence(serialized) {return renderSourceLocalReview(JSON.parse(serialized));}
function keyedPrompt(prompt){
 const boundary=prompt.indexOf('Submit a compact typed review.');assert.ok(boundary>=0);
 return prompt.slice(0,boundary)+' Submit through submit_specialist_review using its review_text string. First line: SUPPORTED only when every assigned claim is supported; otherwise UNSUPPORTED. For supported, provide one line for EVERY assigned C-code, using the syntax C000 S0:1-2 brief source justification. S0:1 also means the single line 1. C000 is the exact assigned code; S0 identifies the supplied immutable file; 1-2 is its inclusive one-based line range. Use C001 EMPTY only to acknowledge an explicitly empty assigned collection. For unsupported, return one to three distinct C-code lines in the same citation syntax, explaining the contradictions; never use EMPTY. Negative excerpts must reconstruct at most 1024 source characters. Positive citations may cover the complete relevant scope. Derive each predicate separately from its caller, including empty, missing, disabled and boundary states. Justify every clause; a true clause cannot rescue a false clause. Keep each reason on one line. Do not put JSON, tables, headings, fences or surrounding prose inside review_text. The application parses these records, checks every original ID and validates exact immutable source before recording the decision.';
}
...
const runtime={async runSession(options){const observe=options.onEvent;return sdk.runSession({...options,userPrompt:readableEvidence(options.userPrompt),systemPrompt:keyedPrompt(options.systemPrompt),customTools:options.customTools?.map(tool=>({...tool,
 ...
 if(tool.name==='submit_specialist_review')acceptedSubmissions.push({...structuredClone(canonical),citation_report:structuredClone(report),assigned_claim_ids:Object.keys(JSON.parse(options.userPrompt).claims),finding_claim_value:JSON.parse(options.userPrompt).claims[canonical.finding?.claim]??null});return result;}})),onEvent(event){if(event.type==='tool_execution_start'&&event.toolName==='submit_specialist_review'){const raw=JSON.stringify(event.args??null);attemptedSubmissions.push({elapsed_ms:Date.now()-started,args:raw.length<=65536?event.args:{truncated:raw.slice(0,65536)}});}if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
