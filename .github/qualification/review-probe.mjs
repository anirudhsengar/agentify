...
const experimentPrompt=fs.readFileSync(path.join(path.dirname(new URL(import.meta.url).pathname),'review-instructions.txt'),'utf8');
const candidate='529c937d46e7ae45876a22f52bd4a820ccd0deab';assert.equal(git(root,'rev-parse','HEAD'),candidate);
...
const runtime={async runSession(options){const observe=options.onEvent;return sdk.runSession({...options,systemPrompt:experimentPrompt,customTools:options.customTools?.map(tool=>({...tool,async execute(...args){const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push({...structuredClone(args[1]),finding_claim_value:JSON.parse(options.userPrompt).claims[args[1].finding?.claim]??null});return result;}})),onEvent(event){if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
