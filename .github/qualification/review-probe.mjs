...
const reviewTasks=[],accepted=[],wireSubmissions=[],sdkCalls=[],events=[],proofSessions=[];
...
const runtime={async runSession(options){
 ...
 const adapted=withPrecheckProofs({...options,customTools:tools,onEvent(event){
 ...
 try{return await sdk.runSession(adapted.options);}
 finally{if(adapted.snapshot())proofSessions.push(...adapted.snapshot());}
}};
...
const result={candidate_sha:selected.candidate_sha,candidate_tree:selected.candidate_tree,case:selected,component_only:true,fresh_installation:false,installation_credit:false,
 production_prompt_unchanged:false,precheck_source_proof_experiment:true,proof_sessions:proofSessions,production_budget_overrides:false,compound_clause_precheck:true,canonical_submission_projection:true,
 ...
 accepted_submissions:accepted,canonical_unit_submissions:wireSubmissions,provider_wire:wire,
