import { supportParameters, normalizeReviewReport, evidenceClaimIds } from './support-protocol.mjs';
...
function readableEvidence(serialized) {
 ...
 const sections=['UNTRUSTED ASSIGNED CLAIMS:',JSON.stringify(data.claims,null,2),'CLAIMS REQUIRING SOURCE JUSTIFICATION:',JSON.stringify(evidenceClaimIds(data)),
 ...
 for(const [sourceId,[file,source]] of Object.entries(data.evidence).entries()){
  ...
  sections.push('IMMUTABLE UNTRUSTED SOURCE S'+sourceId+' '+JSON.stringify(file)+'; assigned references '+JSON.stringify(Object.entries(data.claims).filter(([,value])=>mentions(value,file)).map(([id])=>id)),
...
const runtime={async runSession(options){const observe=options.onEvent;return sdk.runSession({...options,userPrompt:readableEvidence(options.userPrompt),systemPrompt:options.systemPrompt+" A supported checklist alone is insufficient: give one short source-line support_evidence entry per nonempty assigned ID. Empty collections stay in checked_claims without a source justification. For every citation use the displayed S-number and inclusive one-based start_line/end_line; the application reconstructs exact source bytes, so do not copy or invent excerpt text. Explain the entire claim including conditional and quantified clauses. If a clause contradicts the source, return unsupported rather than manufacturing a support proof.",customTools:options.customTools?.map(tool=>({...tool,parameters:supportParameters(tool.parameters,JSON.parse(options.userPrompt)),async execute(...args){const canonical=normalizeReviewReport(args[1],JSON.parse(options.userPrompt));const result=await tool.execute(args[0],canonical,...args.slice(2));if(tool.name==='submit_specialist_review')acceptedSubmissions.push({...structuredClone(canonical),citation_report:structuredClone(args[1]),assigned_claim_ids:Object.keys(JSON.parse(options.userPrompt).claims),finding_claim_value:JSON.parse(options.userPrompt).claims[canonical.finding?.claim]??null});return result;}})),onEvent(event){if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
