...
if(selected.full_review_canary==='one_line')body.one_line='PyJWK.from_json always returns the original input string unchanged without parsing JSON or constructing a key.';
if(selected.full_review_canary==='flows[0]')body.flows[0].description='PyJWK.from_dict always returns None without constructing a PyJWK object or materializing a key.';
...
const wireTap=installProviderArgumentTap();const reviewTasks=[];let fullThinkingEvents=0;
const sdk=new PiSdkRuntime();const sdkCalls=[];
const runtime={async runSession(options){const observe=options.onEvent;const task=readReviewPrompt(options.userPrompt);reviewTasks.push({source_precheck:task.source_precheck===true,source_excerpt:task.source_excerpt===true,claim_ids:Object.keys(task.claims),source_paths:Object.keys(task.evidence),source_bytes:Object.values(task.evidence).reduce((n,s)=>n+Buffer.byteLength(s),0),output_cap:options.maxOutputTokens,timeout_ms:options.timeoutMs,thinking_level:task.source_precheck===true?options.config.thinkingLevel:"off"});return sdk.runSession({...options,config:reviewPhaseConfig(options.config,task.source_precheck===true),customTools:options.customTools?.map(tool=>({...tool,async execute(...args){const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push(structuredClone(args[1]));return result;}})),onEvent(event){if(event.type==='tool_execution_start')sdkCalls.push({id:event.toolCallId,args:event.args});if(event.type==='message_update'){if(task.source_precheck!==true&&event.assistantMessageEvent?.type==='thinking_delta')fullThinkingEvents++;stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
