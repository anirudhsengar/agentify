...
const canaryRisk=selected.required_claim_canary
 ? "options.require is ignored when signature verification is disabled, even when required claims are missing."
 : 'Numeric-string exp claim values are always rejected rather than converted to integers.';
if(selected.false_claim_canary){
 assert.ok(fs.readFileSync(path.join(cwd,'jwt/api_jwt.py'),'utf8').includes('exp = int(payload["exp"])'));
 body.pitfalls.unshift({risk:canaryRisk,consequence:selected.required_claim_canary?"Missing required claims are silently accepted when verify_signature=False.":'Even a valid numeric string can never be checked as an expiry timestamp.' ,reference:'jwt/api_jwt.py'});
}
...
function focusedPrompt(prompt) {
 ...
 return prompt.replace(companion,"After the first decisive finding, submit immediately. Do not supply additional_findings or describe a supported assertion as a finding. Full approval still requires every supplied claim to be supported.");
}
const sdk=new PiSdkRuntime();
const runtime={async runSession(options){const observe=options.onEvent;return sdk.runSession({...options,systemPrompt:focusedPrompt(options.systemPrompt),customTools:options.customTools?.map(tool=>({...tool,parameters:{...tool.parameters,properties:{...tool.parameters.properties,additional_findings:{...tool.parameters.properties.additional_findings,maxItems:0}}},async execute(...args){if(args[1].additional_findings?.length)throw new Error("Submit exactly one decisive finding; omit additional_findings.");const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push({...structuredClone(args[1]),finding_claim_value:JSON.parse(options.userPrompt).claims[args[1].finding?.claim]??null});return result;}})),onEvent(event){if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
