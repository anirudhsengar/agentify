...
assert.ok(result.source_unchanged);if(selected.require_source_excerpt)assert.ok(reviewTasks.some(t=>t.source_precheck&&t.source_excerpt),'Required bounded source excerpt did not execute');if(selected.require_source_precheck)assert.ok(reviewTasks.some(t=>t.source_precheck),'Required source-local precheck did not execute');assert.ok(provider_wire.length>0&&provider_wire.every((r,index)=>r.model==='MiniMax-M3'&&r.tool_choice?.type==='auto'&&[4096,12000].includes(r.max_tokens)&&(reviewTasks[index]?.source_precheck ? r.thinking?.type==='disabled'||!r.thinking : r.thinking?.type==='adaptive')),'Actual wire did not preserve documented thinking mode and output cap');assert.ok(result.positive_control_passed,'Valid cache control was rejected or changed');assert.ok(!result.known_bad_body_approved,'Known source contradiction was approved');if(selected.false_claim_canary)assert.ok(result.canary_rejected,'false numeric-expiry canary was not explicitly rejected');else assert.ok(result.valid_typed_outcome,'bounded review did not reach an explicit typed outcome');
