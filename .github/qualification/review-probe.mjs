...
if(selected.positive_control||selected.full_stage_canary){body={...JSON.parse(fs.readFileSync(new URL('./valid-cache-control.json',import.meta.url),'utf8')),last_updated:body.last_updated};selected.concern=body.concern;}
...
let fullStageClaim=null;
if(selected.full_stage_canary==='summary'){
 body.one_line='The cache get() method returns the stored set even after the cache entry has expired.';
 fullStageClaim='one_line';
}else if(selected.full_stage_canary==='flow'){
 assert.equal(body.flows[1].steps[0].path,'jwt/jwk_set_cache.py');
 body.flows[1].steps[0].what_happens='get() returns None for every stored entry, including present unexpired entries.';
 fullStageClaim='flows[1]';
}
...
const runtime={async runSession(options){const observe=options.onEvent;const task=JSON.parse(options.userPrompt);const reviewConfig=task.source_precheck===true?options.config:{...options.config,thinkingLevel:"off"};reviewTasks.push({source_precheck:task.source_precheck===true,thinking_level:reviewConfig.thinkingLevel,source_excerpt:task.source_excerpt===true,claim_ids:Object.keys(task.claims),source_paths:Object.keys(task.evidence),source_bytes:Object.values(task.evidence).reduce((n,s)=>n+Buffer.byteLength(s),0),output_cap:options.maxOutputTokens,timeout_ms:options.timeoutMs});return sdk.runSession({...options,config:reviewConfig,customTools:options.customTools?.map(tool=>({...tool,async execute(...args){const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push(structuredClone(args[1]));return result;}})),onEvent(event){if(event.type==='tool_execution_start')sdkCalls.push({id:event.toolCallId,args:event.args});if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
...
assert.ok(result.source_unchanged);assert.ok(result.valid_typed_outcome,"Every source control requires a complete typed outcome, not merely an attempted finding");if(selected.require_source_excerpt)assert.ok(reviewTasks.some(t=>t.source_precheck&&t.source_excerpt),'Required bounded source excerpt did not execute');if(selected.require_source_precheck)assert.ok(reviewTasks.some(t=>t.source_precheck),'Required source-local precheck did not execute');assert.ok(provider_wire.length>0&&provider_wire.every(r=>r.model==='MiniMax-M3'&&['adaptive','disabled'].includes(r.thinking?.type)&&r.tool_choice?.type==='auto'&&r.max_tokens===12000),'Actual wire did not preserve documented thinking mode and output cap');assert.ok(result.positive_control_passed,'Valid cache control was rejected or changed');assert.ok(result.full_stage_canary_rejected,'The full review missed its deliberately false summary or flow claim');assert.ok(!result.known_bad_body_approved,'Known source contradiction was approved');if(selected.false_claim_canary)assert.ok(result.canary_rejected,'false numeric-expiry canary was not explicitly rejected');else assert.ok(result.valid_typed_outcome,'bounded review did not reach an explicit typed outcome');
