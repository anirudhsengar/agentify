...
const runtime={async runSession(options){
 ...
 reviewTasks.push({thinking_level:options.config.thinkingLevel,source_precheck:task.source_precheck===true,source_excerpt:task.source_excerpt===true,
  ...
  output_cap:options.maxOutputTokens,timeout_ms:options.timeoutMs,clause_bindings:bindings,review_assignment:task.review_assignment??null,
...
if(selected.known_bad_fragment){
 const parts=reviewTasks.filter(t=>t.review_assignment);
 assert.equal(parts.length,2,'Post-prune large body must complete two assignments');
 assert.ok(parts.every(t=>t.review_assignment.count===2&&t.source_bytes===parts[0].source_bytes&&JSON.stringify(t.source_paths)===JSON.stringify(parts[0].source_paths)),'Both assignments require the same complete immutable source');
 const overlap=parts[0].claim_ids.filter(id=>parts[1].claim_ids.includes(id)).sort();
 assert.deepEqual(overlap,['concern','covers','excludes']);
 const union=new Set(parts.flatMap(t=>t.claim_ids));
 assert.ok(['one_line','validation','entry_questions','pitfalls[0]'].every(id=>union.has(id)),'Partition union omitted required original claims');
}
