...
const submittedBodyDigest=specialistReviewDigest(body);const acceptedSubmissions=[];const acceptedObservations=[];const reuseEvents=[];
...
const runtime={async runSession(options){
 ...
 const customTools=options.customTools?.map(original=>{
  const recorded={...original,async execute(...args){const result=await original.execute(...args);
   if(original.name==='submit_specialist_review')acceptedSubmissions.push({...structuredClone(args[1]),finding_claim:task.claims[args[1].finding?.claim]??null});
   if(original.name==='submit_source_observations')acceptedObservations.push(structuredClone(args[1]));return result;}};
  if(original.name!=='submit_specialist_review')return recorded;
  const batch=createClaimProofTool(recorded,task,Value);batches.push(batch);return batch.tool;
 });
...
const output=await reviewSpecialistCompilation({cwd,runtime,config:MODEL_CONFIG,
 ui:{status(){}},auditLog:{recordMessageEnd(){},sessionEvent(value){
  const e=value.event;
  if(e?.type==='source_observations_reused')reuseEvents.push({...e,elapsed_ms:Date.now()-started});
...
if(selected.require_source_observation)verifySourceFirstTasks(result);
