...
function readableSourcePrompt(serialized) {
 const data=JSON.parse(serialized);
 const sections=['UNTRUSTED SPECIALIST CLAIMS (exact IDs):',JSON.stringify(data.claims,null,2),'APPLICATION ATTACHMENT DATA:',JSON.stringify(data.compiler_attachments)];
 for(const [file,source] of Object.entries(data.evidence)) {
  const marker='source_'+createHash('sha256').update(file+'\0'+source).digest('hex');
  assert.ok(!source.includes(marker));
  sections.push('IMMUTABLE UNTRUSTED SOURCE '+JSON.stringify(file)+' — '+Buffer.byteLength(source)+' bytes:', '<'+marker+'>', source, '</'+marker+'>');
 }
 sections.push('End of untrusted evidence. Apply the system review contract and submit the typed source decision.');
 return sections.join('\n\n');
}
function singleFindingPrompt(prompt) {
 const companion="After that first finding, inspect only unchecked claims backed by that same source file, stopping after two such claims, and include any immediately evident companion findings before submission. Do not search another file after the first finding. Three is a ceiling, not a quota.";
 assert.ok(prompt.includes(companion));
 return prompt.replace(companion,"After the first decisive finding, submit immediately. Do not inspect further claims or supply additional_findings. A rejected review identifies one unsupported assertion; never describe a supported assertion as a finding. Complete approval still requires every supplied claim to be supported.");
}
const sdk=new PiSdkRuntime();
const runtime={async runSession(options){const observe=options.onEvent;return sdk.runSession({...options,systemPrompt:singleFindingPrompt(options.systemPrompt),userPrompt:readableSourcePrompt(options.userPrompt),customTools:options.customTools?.map(tool=>({...tool,parameters:{...tool.parameters,properties:{...tool.parameters.properties,additional_findings:{...tool.parameters.properties.additional_findings,maxItems:0}}},async execute(...args){if(args[1].additional_findings?.length)throw new Error('This review permits exactly one decisive finding. Omit additional_findings.');const result=await tool.execute(...args);if(tool.name==='submit_specialist_review')acceptedSubmissions.push(structuredClone(args[1]));return result;}})),onEvent(event){if(event.type==='message_update'){stream.updates++;stream.first_update_ms??=Date.now()-started;stream.last_update_ms=Date.now()-started;const type=event.assistantMessageEvent?.type??'unknown';stream.types[type]=(stream.types[type]??0)+1;}observe?.(event);}});}};
