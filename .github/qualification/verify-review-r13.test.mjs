...
for(const scenario of ['valid','source','tree','node','qualification-failed','wrong-producer','wrong-artifact','unreviewed','canary-missed','source-changed','claims-leaked','missing-full-review','note-verdict','fractional-citation','altered-thinking','no-observation-terminal','second-round-failed']){
 test('exact-candidate installation evidence gate: '+scenario,()=>{
  ...
  try{
   write('qualification.json',JSON.stringify({id:34248174975,status:'completed',conclusion:scenario==='qualification-failed'?'failure':'success',head_sha:scenario==='wrong-producer'?'e'.repeat(40):launcher,run_attempt:1}));
   for(const node of ['22.19.0','24.19.0']){
    const root='qualified-source/node-'+node+'/pr13-source-first-r13-code-';
   ...
   for(const name of ["r13-1-high-jwk", "r13-1-high-valid-cache", "r13-1-numeric-exp-canary", "r13-2-high-jwk", "r13-2-high-valid-cache", "r13-2-numeric-exp-canary"]){
    ...
    const value={candidate_sha:candidate,candidate_tree:tree,source_unchanged:scenario!=='source-changed',valid_typed_outcome:scenario!=='unreviewed'&&!(scenario==='second-round-failed'&&name.startsWith('r13-2-')),production_budget_overrides:false,
   ...
   const run=spawnSync(process.execPath,[new URL('./verify-review-r13.mjs',import.meta.url).pathname],{env:{...process.env,RUNNER_TEMP:temp,EXPECTED_CANDIDATE:candidate,EXPECTED_TREE:tree,EXPECTED_LAUNCHER:launcher,GITHUB_ENV:envFile},encoding:'utf8',timeout:5000});
