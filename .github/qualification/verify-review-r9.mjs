...
for(const node of ['22.19.0','24.19.0']){
 const dir=path.join(root,'node-'+node);const env=fs.readFileSync(path.join(dir,'pr13-initial-trace-r9b-environment.txt'),'utf8');
 assert.ok(env.includes('candidate='+candidate+'\n'));assert.ok(env.includes('tree='+tree+'\n'));
 assert.ok(env.includes('v'+node+'\n11.19.0\n'));assert.equal(fs.readFileSync(path.join(dir,'pr13-initial-trace-r9b-qualification.exit'),'utf8').trim(),'0');
 const log=fs.readFileSync(path.join(dir,'pr13-initial-trace-r9b-qualification.log'),'utf8');
...
for(const name of ['r9-high-jwk','r9-high-valid-cache','r9-numeric-exp-canary']){
 ...
 if(name==='r9-high-valid-cache'){assert.equal(result.positive_control_passed,true);assert.equal(result.review.failure,null);}
 if(name==='r9-high-jwk'){
 ...
 if(name==='r9-numeric-exp-canary'){assert.equal(result.false_claim_canary,true);assert.equal(result.canary_rejected,true);}
}
fs.appendFileSync(process.env.GITHUB_ENV,'LIVE_EXPECTED_PACKAGE_HASH='+digest+'\n');
fs.writeFileSync(path.join(temp,'review-r9-qualification-reuse.json'),JSON.stringify({candidate,tree,qualification_run:run.id,launcher,package_sha256:digest,code_nodes:['22.19.0','24.19.0'],source_controls:3},null,2)+'\n');
