...
for(const node of ['22.19.0','24.19.0']){
 const dir=path.join(root,'node-'+node);const env=fs.readFileSync(path.join(dir,'pr13-balanced-r12-code-environment.txt'),'utf8');
 assert.ok(env.includes('candidate='+candidate+'\n'));assert.ok(env.includes('tree='+tree+'\n'));
 assert.ok(env.includes('v'+node+'\n11.19.0\n'));assert.equal(fs.readFileSync(path.join(dir,'pr13-balanced-r12-code-qualification.exit'),'utf8').trim(),'0');
 const log=fs.readFileSync(path.join(dir,'pr13-balanced-r12-code-qualification.log'),'utf8');
...
for(const name of ["r12-1-high-jwk", "r12-1-high-valid-cache", "r12-1-numeric-exp-canary", "r12-2-high-jwk", "r12-2-high-valid-cache", "r12-2-numeric-exp-canary"]){
 ...
 verifyBalancedAssignments(result);
 ...
 if(name.endsWith('-high-jwk')){
  assert.equal(result.known_bad_body_approved,false);assert.ok(result.review_tasks.some(task=>task.assignment?.focus));
...
fs.writeFileSync(path.join(temp,'review-r12-qualification-reuse.json'),JSON.stringify({candidate,tree,qualification_run:run.id,launcher,package_sha256:digest,code_nodes:['22.19.0','24.19.0'],source_controls:6},null,2)+'\n');
