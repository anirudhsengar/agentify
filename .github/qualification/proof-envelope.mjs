...
import {supportParameters,normalizeReviewReport,claimBindings} from './support-protocol.mjs';
...
export function parseProofEnvelope(input,originalParameters,data,value){
 ...
 const support={};const findings=[];const seen=new Set();const errors=[];
 for(const line of lines){
  const empty=/^(C[0-9]{3})[ \t]+EMPTY$/.exec(line);
  const match=/^(C[0-9]{3})[ \t]+S([0-9]+):([0-9]+)(?:-([0-9]+))?[ \t]+(.+)$/.exec(line);
  if(!empty&&!match){errors.push('Invalid record: '+line.slice(0,160));continue;}
  const claim=(empty??match)[1];if(seen.has(claim)){errors.push('Duplicate claim record: '+claim);continue;}seen.add(claim);
  if(empty){assert.equal(header,'SUPPORTED','An unsupported assertion needs source evidence.');support[claim]=true;continue;}
  const record={source:Number(match[2]),start_line:Number(match[3]),end_line:Number(match[4]??match[3]),reason:match[5]};
  if(header==='SUPPORTED')support[claim]=record;else findings.push({claim,...record});
 }
 if(header==='SUPPORTED'){
  for(const [code,id] of Object.entries(claimBindings(data)))if(!seen.has(code))errors.push('Missing '+code+(Array.isArray(data.claims[id])&&data.claims[id].length===0?' EMPTY (explicitly acknowledge the supplied empty collection)':' source justification'));
 }
 ...
 if(!value.Check(schema,report)){
  errors.push(...[...value.Errors(schema,report)].slice(0,3).map(e=>e.instancePath+': '+e.message));
 }
 if(errors.length)throw new Error(('Invalid decoded review: '+errors.slice(0,8).join('; ')+'. Return the complete corrected review_text, not a delta; preserve every assigned C-code.').slice(0,2048));
