...
for(const scenario of ['valid','source','tree','node','qualification-failed','wrong-producer','wrong-artifact','unreviewed','canary-missed','source-changed','claims-leaked','missing-full-review','note-verdict','fractional-citation','altered-thinking','no-observation-terminal','second-round-failed','clause-checks-missing']){
 test('exact-candidate installation evidence gate: '+scenario,()=>{
  ...
  try{
   write('qualification.json',JSON.stringify({id:34255071445,status:'completed',conclusion:scenario==='qualification-failed'?'failure':'success',head_sha:scenario==='wrong-producer'?'e'.repeat(40):launcher,run_attempt:1}));
   for(const node of ['22.19.0','24.19.0']){
    const root='qualified-source/node-'+node+'/pr13-compound-r15-code-';
   ...
   for(const name of ["r15-1-high-jwk", "r15-1-high-valid-cache", "r15-1-numeric-exp-canary", "r15-2-high-jwk", "r15-2-high-valid-cache", "r15-2-numeric-exp-canary"]){
    const tasks=[{source_observation:true,claim_ids:[],thinking_level:'high',output_cap:12000,timeout_ms:90000},
      {source_observation:false,claim_ids:['concern','one_line','validation','clause:invariants[4].rule:0','clause:invariants[4].rule:1'],thinking_level:'high',output_cap:12000,timeout_ms:85000,untrusted_source_observations:[]}];
    ...
    if(scenario==='clause-checks-missing')tasks[1].claim_ids=tasks[1].claim_ids.filter(id=>!id.startsWith('clause:'));
    ...
    const value={candidate_sha:candidate,candidate_tree:tree,source_unchanged:scenario!=='source-changed',valid_typed_outcome:scenario!=='unreviewed'&&!(scenario==='second-round-failed'&&name.startsWith('r15-2-')),production_budget_overrides:false,
   ...
   const run=spawnSync(process.execPath,[new URL('./verify-review-r15.mjs',import.meta.url).pathname],{env:{...process.env,RUNNER_TEMP:temp,EXPECTED_CANDIDATE:candidate,EXPECTED_TREE:tree,EXPECTED_LAUNCHER:launcher,GITHUB_ENV:envFile},encoding:'utf8',timeout:5000});
