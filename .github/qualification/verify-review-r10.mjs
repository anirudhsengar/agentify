...
for(const node of ['22.19.0','24.19.0']){
 const dir=path.join(root,'node-'+node);const env=fs.readFileSync(path.join(dir,'pr13-readable-review-r10-code-environment.txt'),'utf8');
 assert.ok(env.includes('candidate='+candidate+'\n'));assert.ok(env.includes('tree='+tree+'\n'));
 assert.ok(env.includes('v'+node+'\n11.19.0\n'));assert.equal(fs.readFileSync(path.join(dir,'pr13-readable-review-r10-code-qualification.exit'),'utf8').trim(),'0');
 const log=fs.readFileSync(path.join(dir,'pr13-readable-review-r10-code-qualification.log'),'utf8');
...
for(const name of ['r10-high-jwk','r10-high-valid-cache','r10-numeric-exp-canary']){
 ...
 if(name==='r10-high-valid-cache'){assert.equal(result.positive_control_passed,true);assert.equal(result.review.failure,null);}
 if(name==='r10-high-jwk'){
 ...
 if(name==='r10-numeric-exp-canary'){assert.equal(result.false_claim_canary,true);assert.equal(result.canary_rejected,true);}
}
fs.appendFileSync(process.env.GITHUB_ENV,'LIVE_EXPECTED_PACKAGE_HASH='+digest+'\n');
fs.writeFileSync(path.join(temp,'review-r10-qualification-reuse.json'),JSON.stringify({candidate,tree,qualification_run:run.id,launcher,package_sha256:digest,code_nodes:['22.19.0','24.19.0'],source_controls:3},null,2)+'\n');
