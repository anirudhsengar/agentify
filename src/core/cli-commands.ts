...
export function printSubcommandHelp(out: NodeJS.WritableStream): void {
  ...
  out.write(`  agentify engage <init|status|validate|report|promotion|metrics|shadow> [options]\n`);
  ...
  out.write(`\nSupported local shadow subcommand (Milestone 6A-L):\n`);
  out.write(`  agentify engage shadow run-local --id <id> --issue <n> --repo <owner/name>\n`);
  out.write(`       [--suite <id>] [--task <id>] --pilot-root <abs> [--yes]\n`);
  out.write(`    Run a read-only local shadow analysis backed by the Agentify\n`);
  out.write(`    shared engine. Persists evidence to the private pilot workspace.\n`);
  out.write(`  agentify engage shadow status-local --id <id> --pilot-root <abs>\n`);
  out.write(`    Inspect local shadow runs and locks beneath the pilot workspace.\n`);
