...
export function renderEvalReport(result: EvalResult): string {
  ...
  return [
    ...
    `- Live shadow trials: ${result.live_shadow_trials ?? 0}`,
    `- Shadow evidence: ${result.shadow_evidence_classification ?? "not applicable"}`,
    ...
    "- Implementation success: not established by shadow-only evidence",
