...
export interface SpinnerHandle {
  ...
  /**
   * Stop the spinner and append a final line. Errors use Clack's error state;
   * other terminal states use its normal completion state.
   */
...
export function startSpinner(initialMessage: string): SpinnerHandle {
  ...
  return {
    ...
    stop(message: string, kind: SpinnerKind = "success"): void {
      ...
      if (kind === "error") sp.error(message);
      else sp.stop(message);
    },
    raw: sp,
  };
}
