...
### D3_type_contract
- **Closure**: enumerate every observed Pydantic/TS/ORM/schema contract with
  real file paths. One real interface/model/schema is sufficient in a small
  repository; otherwise record a real stable type or one end-to-end type trace.
  When the parent applies the report with `write_map_delta`, it should use the
  typed top-level `observed_type_contract: { kind, path, name, fields }`
  parameter for a TypeScript interface or Pydantic model.
...
- **Common gaps**: dynamically-typed codebase (no Pydantic, no
 ...
 `dict` shapes, JSON Schema, Protobuf). Never leave every contract field empty
 when a named type exists. If no contract evidence is observable, the dimension
 remains `gap`; do not claim empty type data is covered.
...
- **Closure**: path classifications (zero-access/read-only/
 ...
 interpreters, env allowlist, security checklist, and at least one explicit
...
- **Minimal exploration**: read `.gitignore`, grep for
 ...
 invocations.
- **Common gaps**: no `.gitignore`, no permissions file, and no declared rules.
 Record the absence honestly and keep the dimension `gap`. Do not claim D8 is
 covered while both `bash_blocked_patterns` and `damage_control_rules` are empty.
