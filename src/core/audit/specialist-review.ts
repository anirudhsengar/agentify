...
import { createSpecialistReviewSubmissionSchema, SourceObservationSubmissionSchema,
  type SourceObservation, type SpecialistReviewSubmission } from "./schema/specialist-review.ts";
...
async function observeSourceTask(
  ...
  const tool = defineTool({
    ...
    description: "Return up to four observed behaviors with exact source excerpts. These notes cannot approve or reject a specialist.",
    ...
    async execute(_id, report) {
      ...
      const checked = report.observations.map(observation => {
        const excerpt = exactSourceExcerpt(task.sources.get(observation.path), observation.excerpt);
        if (excerpt === null) throw new Error("source observation must quote exact supplied source");
  ...
  try {
    ...
    const result = await context.runtime.runSession({
      ...
      config: { ...context.config, thinkingLevel: "off" }, modelRole: "primary",
      ...
      timeoutMs: duration, inactivityTimeoutMs: duration, maxOutputTokens: 4_096,
