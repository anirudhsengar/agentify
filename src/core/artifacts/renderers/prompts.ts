...
function renderIssueTypePrompt(map: CodebaseMap, issueType: string, context: RenderContext): RenderedArtifact {
  ...
  return markdownArtifact({
    ...
    body: [
      ...
      `2. Read \`${context.stateDir}/conditional_docs.md\` and load matching docs when present.`,
...
function renderPerAreaPrompt(
  ...
  return markdownArtifact({
    ...
    body: [
      ...
      "## Goal",
      "",
      `Write a build spec for $ARGUMENTS using \`${context.stateDir}/prompts/${candidate.issue_type}.md\` as the base format, enriched with ${candidate.area_name} context.`,
      "",
      "## Area Context",
      "",
      ...
      "## Workflow",
      "",
      `1. Read \`${context.stateDir}/prompts/${candidate.issue_type}.md\` for the base spec flow.`,
      `2. Read \`${candidate.source_feature_agent}\` for local conventions, key files, and pitfalls.`,
      `3. Read \`AGENTS.md\` and \`${context.stateDir}/conditional_docs.md\`.`,
