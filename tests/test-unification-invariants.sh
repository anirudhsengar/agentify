...
# GreenField+agentify merge depends on (see docs/).
set -euo pipefail
...
for skill_file in packaged/skills/*/SKILL.md; do
...
while IFS= read -r s; do
  [ -f "packaged/skills/$s/SKILL.md" ] || fail "skills-lock.json references missing skill '$s'"
done < <(jq -r '.skills | keys[]' skills-lock.json)

# 3. Forked/reconciled skills must NOT be in the lock: once edited,
#    a skill is agentify-owned and no longer mergeable with upstream.
...
# 4. The package ships skills under packaged/skills/ and nothing under
#    .agents/ or .claude/ at the repo root. Both dotfolders are absent
#    so the maintainer's coding agent (regardless of harness) never
#    auto-loads the shipped build chain on session start. The dual-
#    skill-discovery mirror (.claude/skills/ + .agents/skills/) is
#    generated by the installer into each *target* repo
#    (artifact-exporters.ts::exportClaude / exportCodex), not committed
#    here.
[ ! -e .agents ] || fail ".agents/ must not exist in the maintainer repo (use packaged/skills/)"
[ ! -e .claude ] || fail ".claude/ must not exist in the maintainer repo (use .gitignore + installer)"

# 5. The unification renamed the product: no stale GreenField tokens in the
#    shipped skills or the scaffold payload.
if grep -rnE 'greenfield|GreenField' packaged/skills scaffold 2>/dev/null \
...
# (Live surface only.)
if grep -rn 'ask-greenfield' packaged/skills scaffold 2>/dev/null; then
...
#    (the build spec command is /spec; /plan would collide with /to-plan).
[ -d packaged/skills/plan ] && fail "skill 'plan' must not exist — the build-spec command is /spec"

# 8. (No ADR set in this repo.)
...
# 13. The audit is retargeted to emit intelligence only: the builder
#     prompt must carry the Emission Contract and must not instruct emitting the
#     shipped build chain.
builder=src/core/audit/prompts/builder.md
grep -q "Emission contract" "$builder" || fail "builder.md is missing the Emission Contract"
...
#      must be implemented in src/core/cli-commands.ts (amended
#      2026-07-09).
