...
test("independent readers overlap while duplicate concerns and aggregate overflow remain blocked", async () => {
  ...
  try {
    ...
    runs.push(start("Payment retry contracts"), start("Session invalidation"));
    ...
    assert.equal(sessions, 2, "duplicate/overflow dispatch cannot reach the provider");
    release();
    await Promise.all(runs);
    assert.equal(budget.snapshot().turns, 1);
    assert.equal(budget.snapshot().unreported_calls, 1);
    assert.equal(budget.snapshot().reserved_input_tokens, 100,
