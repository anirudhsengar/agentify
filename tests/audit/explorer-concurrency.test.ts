...
test("four default-budget readers overlap while duplicate concerns and a fifth remain blocked", async () => {
  ...
  try {
    ...
    const budget = new AuditResourceBudget({ maxModelCalls: 4 });
    let sessions = 0;
    let admitted!: () => void;
    ...
    runs.push(start("Payment retry contracts"), start("Session invalidation"), start("Route matching strategies"),
      start("Response rendering contracts"));
    ...
    assert.equal(budget.snapshot().unreported_calls, 4);
    ...
    assert.equal(sessions, 4, "duplicate/overflow dispatch cannot reach the provider");
    release();
    await Promise.all(runs);
    assert.equal(budget.snapshot().turns, 1);
    assert.equal(budget.snapshot().unreported_calls, 3);
    assert.equal(budget.snapshot().reserved_input_tokens, 60,
