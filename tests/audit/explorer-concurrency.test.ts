...
test("independent readers overlap while duplicate concerns and aggregate overflow remain blocked", async () => {
  ...
  try {
    ...
    runs.push(start("Payment retry contracts"), start("Session invalidation"), start("Discovery", "concern_scout"));
    ...
    assert.match(JSON.stringify(await start("Another discovery", "concern_scout")), /already active/);
