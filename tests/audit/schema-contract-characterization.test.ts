...
function testSerializedSchemaContract(): void {
  ...
  const observedContractProps = properties(deltaProps.observed_type_contract ?? {});
  ...
  assert.deepEqual(Object.keys(observedContractProps), ["kind", "path", "name", "fields"]);
  assert.deepEqual(observedContractProps.kind?.enum, ["typescript_interface", "pydantic_model"]);
  assert.equal(observedContractProps.fields?.minItems, 1);
  assert.equal(observedContractProps.fields?.maxItems, 64);
  assert.deepEqual(
...
const tests: Array<{ name: string; run: () => void }> = [
  ...
  { name: "schema facade identity", run: testFacadeIdentity },
