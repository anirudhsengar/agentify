...
const common = { schema_version: "1" as const, engagement_id: "eng", workflow_id: "flow", run_id: "run-1", timestamp: "2026-07-22T00:00:00.000Z", source: "runtime" as const, execution_origin: "github_live_shadow" as const, provenance: { quality: "measured" as const, method: "test runtime", source_reference: "evidence.json" }, evidence_references: ["evidence.json"], redaction_status: "reference_only" as const };
...
test("new metric events require explicit execution origin", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agentify-metrics-origin-"));
  const { execution_origin: _origin, ...missing } = started();
  assert.throws(() => recordMetricEvent(root, missing as MetricEventInput), /require explicit execution_origin/);
});

test("legacy metric events without explicit origin are preserved as legacy_unspecified", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agentify-metrics-legacy-"));
  const directory = path.join(root, "engagements/eng/metrics");
  fs.mkdirSync(directory, { recursive: true });
  const { execution_origin: _origin, ...legacy } = { ...started(), event_id: "f".repeat(64) };
  fs.writeFileSync(path.join(directory, "run-events.jsonl"), `${JSON.stringify(legacy)}\n`);
  const events = readMetricEvents(root, "eng");
  assert.equal(events[0]?.execution_origin, "legacy_unspecified");
  assert.equal(aggregatePilotEvents(events).runs_by_execution_origin.legacy_unspecified.started, 1);
});
