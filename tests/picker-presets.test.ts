...
async function balancedPutsMediumInExplorerAndLite(): Promise<void> {
  const strongest = model("claude-opus-4-8", true, 1_000_000);
  const medium = model("claude-sonnet-4-6", true, 500_000);
  const fast = model("claude-haiku-4-5-20251001", true, 200_000);
...
async function costOptimizedPutsFastInExplorerAndLite(): Promise<void> {
  const strongest = model("claude-opus-4-8", true, 1_000_000);
  const medium = model("claude-sonnet-4-6", true, 500_000);
  const fast = model("claude-haiku-4-5-20251001", true, 200_000);
...
const tests: Array<{ name: string; fn: () => Promise<void> }> = [
  ...
  { name: "balancedPutsMediumInExplorerAndLite", fn: balancedPutsMediumInExplorerAndLite },
  { name: "costOptimizedPutsFastInExplorerAndLite", fn: costOptimizedPutsFastInExplorerAndLite },
