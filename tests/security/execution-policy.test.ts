...
async function testReadOnlyToolAdmission(): Promise<void> {
  ...
  try {
    ...
    assert.equal(policy.networkIsolation, "not-provided");
    assert.equal("network" in policy, false);
...
async function testShellEscapeCasesBlocked(): Promise<void> {
  const cwd = tempDir("agentify-policy-shell-");
  try {
    const hook = makeDefenseHook({
      executionPolicy: createReadOnlyExecutionPolicy({ cwd }),
    });
    for (const command of [
      ...
      assert.ok(result?.block, `execution policy must block: ${command}`);
    }
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
...
const tests: Array<{ name: string; fn: () => Promise<void> }> = [
  ...
  { name: "shellEscapeCasesBlocked", fn: testShellEscapeCasesBlocked },
];
