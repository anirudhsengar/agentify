...
const statusBeforeChecks = worktreeStatus();
...
for (const check of config.validation_checks) {
  ...
  checks.push({ name: check.name.slice(0, 120), kind: check.kind, status: result.error?.code === "ETIMEDOUT" ? "timeout" : result.status === 0 ? "passed" : "failed", exit_code: result.status, runtime_ms: Date.now() - started, output_tail: "Command output omitted from persisted evidence to prevent secret leakage." });
}
...
const headAfterChecks = execFileSync("git", ["-C", root, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const statusAfterChecks = worktreeStatus();
...
const policyChecks = { forbidden_paths: forbiddenChanges.length === 0, dependency_changes: allowDependencies || dependencyChanges.length === 0, generated_file_ownership: generatedOwnershipFailures.length === 0, diff_nonempty: changed.length > 0, validation_head_unchanged: headAfterChecks === head, validation_worktree_unchanged: JSON.stringify(statusAfterChecks) === JSON.stringify(statusBeforeChecks), cost: usage.cost_usd <= Number(config.maximum_cost_usd), runtime: usage.runtime_ms <= Number(config.maximum_runtime_ms) };
const checksPassed = checks.every((check) => check.status === "passed"); const safetyPolicyPassed = Object.values(policyChecks).every(Boolean);
const passed = checksPassed && safetyPolicyPassed; const failedDraftPolicyUsed = !checksPassed && safetyPolicyPassed && config.allow_failed_draft === true;
const report = { schema_version: "1", base_commit: base, head_commit: head, files_changed: changed, validation_results: checks, diff_policy: policyChecks, forbidden_path_changes: forbiddenChanges, dependency_changes: dependencyChanges, generated_ownership_failures: generatedOwnershipFailures, validation_status_before: statusBeforeChecks, validation_status_after: statusAfterChecks, cost_usd: usage.cost_usd, cost_source: typeof usage.cost_source === "string" ? usage.cost_source : "measured", runtime_ms: usage.runtime_ms, retries: Number.isInteger(usage.retries) ? usage.retries : 0, passed, publication_allowed: passed || failedDraftPolicyUsed, failed_draft_policy_used: failedDraftPolicyUsed };
