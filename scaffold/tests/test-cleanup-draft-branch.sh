...
cat > "$tmp/bin/gh" <<'EOF'
#!/usr/bin/env bash
if [ "$1 $2" = "repo view" ]; then echo "${DEFAULT_BRANCH:-main}"; elif [ "$1" = api ]; then if [ "${PROTECTION_ERROR:-0}" = 1 ]; then echo 'HTTP 403: forbidden' >&2; exit 1; elif [ "${PROTECTED:-0}" = 1 ]; then echo 'HTTP/2.0 200'; else echo 'HTTP/2.0 404' >&2; exit 1; fi; elif [ "$1 $2" = "pr list" ]; then echo "${ACTIVE_PRS:-0}"; fi
EOF
cat > "$tmp/bin/git" <<'EOF'
#!/usr/bin/env bash
...
if PROTECTION_ERROR=1 env "${envs[@]}" bash "$cleanup" "$state" agent/draft-42-run-change --confirm >/dev/null 2>&1; then exit 1; fi
